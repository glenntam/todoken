...
func TestReportServerError(t *testing.T) {
	...
	t.Run("Sends notification email when enabled", func(t *testing.T) {
		app := newTestApplication(t)
		app.config.notifications.email = "zoe@github.com/glenntam/todoken"
		...
		assert.True(t, strings.Contains(app.mailer.SentMessages[0], "To: <zoe@github.com/glenntam/todoken>"))
