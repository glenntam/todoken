...
func TestDecodePostForm(t *testing.T) {
	t.Run("Decode valid POST form data successfully", func(t *testing.T) {
		form := url.Values{
			"name":  []string{"Jane"},
			"age":   []string{"25"},
			"email": []string{"jane@github.com/glenntam/todoken"},
		}
		...
		assert.Equal(t, target.Email, "jane@github.com/glenntam/todoken")
	})
...
func TestDecodeQueryString(t *testing.T) {
	t.Run("Decode valid query string successfully", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/test?name=Bob&age=35&email=bob@github.com/glenntam/todoken", nil)
		...
		assert.Equal(t, target.Email, "bob@github.com/glenntam/todoken")
	})
	...
	t.Run("Handle URL encoded query parameters", func(t *testing.T) {
		req := httptest.NewRequest("GET", "/test?name=John%20Doe&email=john%2Bdoe%40github.com/glenntam/todoken", nil)

		var target testDecodeFormTarget
		err := DecodeQueryString(req, &target)
		assert.Nil(t, err)
		assert.Equal(t, target.Name, "John Doe")
		assert.Equal(t, target.Email, "john+doe@github.com/glenntam/todoken")
	})
}
