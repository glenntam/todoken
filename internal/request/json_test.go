...
func TestDecodeJSON(t *testing.T) {
	t.Run("Decode valid JSON successfully", func(t *testing.T) {
		jsonBody := `{"name":"John","age":30,"email":"john@github.com/glenntam/todoken"}`
		...
		assert.Equal(t, target.Email, "john@github.com/glenntam/todoken")
	})

	t.Run("Allow unknown fields", func(t *testing.T) {
		jsonBody := `{"name":"John","age":30,"email":"john@github.com/glenntam/todoken","unknown_field":"value"}`
		req := httptest.NewRequest("POST", "/test", strings.NewReader(jsonBody))
		w := httptest.NewRecorder()
	...
	t.Run("Return error for incorrect JSON type", func(t *testing.T) {
		jsonBody := `{"name":"John","age":"not-a-number","email":"john@github.com/glenntam/todoken"}`
		req := httptest.NewRequest("POST", "/test", strings.NewReader(jsonBody))
		w := httptest.NewRecorder()
